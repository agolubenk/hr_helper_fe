...
| 1 | Кандидат | ats | ATS \| Talent Pool | /ats/vacancy/1/candidate/1 | |
...
- **topOffset:** на странице `/ats` — 112px (Header 64px + StatusBar 48px), на остальных — 64px.
...
- **recruiting** — активен на `/workflow`, `/ats*`, `/invites*`, `/vacancies*`, `/hiring-requests*`, `/interviewers*`.
...
- `marginTop`: 112px для `/ats`, 64px для остальных.
...
Ссылки в меню ведут сюда, но маршрутов нет: `/ats/*`, `/invites`, `/specializations*`, `/projects*`, `/employees*`, `/onboarding/*`, `/performance/*`, `/learning/*`, `/hr-services/*`, `/huntflow`, `/telegram*`, `/reporting/*` (кроме `/reporting`), `/analytics/*`, `/compensation/benefits`. Часть разделов доступна в админке по `/admin/...`.
