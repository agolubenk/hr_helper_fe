/**
 * Оригинальные иконки брендов интеграций.
 * Источники: официальные сайты (hh.ru, telegram.org, n8n.io, notion.so, clickup.com)
 */
import type { ReactNode } from 'react'

interface IconProps {
  size?: number
  style?: React.CSSProperties
}

const defaultSize = 16

/** HeadHunter + Rabota.by — иконка, разделённая по диагонали (HH | Rabota.by) */
export function HHIcon({ size = defaultSize, style }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" style={style}>
      <defs>
        {/* Левая половина по диагонали (верхний левый треугольник) */}
        <clipPath id="hh-rabota-left">
          <path d="M0 0 L40 0 L0 40 Z" />
        </clipPath>
        {/* Правая половина по диагонали (нижний правый треугольник) */}
        <clipPath id="hh-rabota-right">
          <path d="M40 0 L40 40 L0 40 Z" />
        </clipPath>
      </defs>
      <g transform="translate(20,20) scale(0.911) translate(-20,-20)">
      {/* HH (HeadHunter) — левая половина, ширина +20%, форма полукруга */}
      <g clipPath="url(#hh-rabota-left)">
        <g transform="translate(20,20) scale(1.037) translate(-20,-20)">
        <path
          d="M20 40c11.046 0 20-8.954 20-20S31.046 0 20 0 0 8.954 0 20s8.954 20 20 20z"
          fill="#000"
        />
        <path
          d="M29.73 17.05c-.605-.624-1.465-.965-2.515-.965-1.305 0-2.307.53-2.884 1.523V12.68h-3.026v13.504h3.026v-4.748c0-1.106.227-1.815.568-2.231.33-.416.794-.577 1.295-.577.445 0 .794.142 1.04.397.246.265.388.672.388 1.239v5.91h3.026v-6.506c0-1.106-.322-2.005-.917-2.62zm-13.531-.965c-1.305 0-2.308.53-2.884 1.523V12.68h-3.026v13.504h3.026v-4.748c0-1.106.226-1.815.567-2.231.33-.416.794-.577 1.296-.577.444 0 .794.142 1.04.397.245.265.387.672.387 1.239v5.91h3.026v-6.506c0-1.106-.321-2.005-.926-2.629-.596-.624-1.457-.955-2.506-.955z"
          fill="#fff"
        />
        </g>
      </g>
      {/* Rabota.by — правая половина, −12% (https://i.rabota.by/styles/images/logos/rabota.by.mobile__min_.svg) */}
      <g clipPath="url(#hh-rabota-right)">
        <g transform="translate(20,20) scale(0.88) translate(-20,-20)">
        <path
          d="M30.469 0C36.25 0 40 3.75 40 9.531V30.47c0 5.78-3.75 9.53-9.531 9.53H9.53C3.75 40 0 36.25 0 30.469V9.53C0 3.75 3.75 0 9.531 0H30.47z"
          fill="#E80200"
        />
        <path
          d="M31.5 17.546c0-4.17-3.799-7.546-8.489-7.546H9v20h5.727v-4.91h6.867l3.77 4.91H31.5l-4.47-5.805c2.658-1.286 4.47-3.78 4.47-6.65zM23.011 20h-8.284v-4.91h8.284c1.52 0 2.762 1.105 2.762 2.456 0 1.35-1.242 2.454-2.762 2.454z"
          fill="#fff"
        />
        </g>
      </g>
      {/* Чёрная линия по диагонали, длина −15% −8% (диаметр линии) */}
      <line x1="4.4" y1="35.6" x2="35.6" y2="4.4" stroke="#000" strokeWidth="0.6" strokeLinecap="square" />
      </g>
    </svg>
  )
}

/** Telegram — бумажный самолёт (официальный логотип) */
export function TelegramIcon({ size = defaultSize, style }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={style}>
      <path
        d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.562 8.161c-.18 1.897-.962 6.502-1.359 8.627-.168.9-.5 1.201-.82 1.23-.697.064-1.226-.461-1.901-.903-1.056-.692-1.653-1.123-2.678-1.799-1.185-.781-.417-1.21.258-1.911.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.139-5.062 3.345-.479.329-.913.489-1.302.481-.428-.009-1.252-.242-1.865-.442-.751-.244-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.831-2.529 6.998-3.015 3.333-1.386 4.025-1.627 4.477-1.635.099-.002.321.023.465.141.121.1.154.234.17.331.015.098.034.321.019.495z"
        fill="currentColor"
      />
    </svg>
  )
}

/** n8n — иконка с кругами (официальный логотип) */
export function N8nIcon({ size = defaultSize, style }: IconProps) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 87 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      preserveAspectRatio="xMidYMid meet"
      style={{ display: 'block', ...style }}
    >
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M47.9855 4.8C47.9855 7.45098 45.8365 9.6 43.1855 9.6C40.949 9.6 39.0696 8.0703 38.5368 6H31.8352C30.662 6 29.6607 6.84822 29.4678 8.00544L29.2706 9.18912C29.0832 10.313 28.5147 11.2911 27.7108 12C28.5147 12.7089 29.0832 13.687 29.2706 14.8109L29.4678 15.9946C29.6607 17.1518 30.662 18 31.8352 18H33.7368C34.2696 15.9297 36.149 14.4 38.3855 14.4C41.0365 14.4 43.1855 16.549 43.1855 19.2C43.1855 21.851 41.0365 24 38.3855 24C36.149 24 34.2696 22.4703 33.7368 20.4H31.8352C29.4888 20.4 27.4863 18.7036 27.1005 16.3891L26.9032 15.2054C26.7104 14.0482 25.7091 13.2 24.5359 13.2H22.5782C21.979 15.1681 20.1495 16.6 17.9855 16.6C15.8216 16.6 13.9921 15.1681 13.3929 13.2H10.5782C9.97901 15.1681 8.14949 16.6 5.98554 16.6C3.33458 16.6 1.18555 14.4509 1.18555 11.8C1.18555 9.14904 3.33458 7.00002 5.98554 7.00002C8.29361 7.00002 10.2212 8.62902 10.6812 10.8H13.2899C13.7499 8.62902 15.6775 7.00002 17.9855 7.00002C20.2936 7.00002 22.2212 8.62902 22.6812 10.8H24.5359C25.7091 10.8 26.7104 9.95178 26.9032 8.79456L27.1005 7.61088C27.4863 5.29638 29.4888 3.6 31.8352 3.6H38.5368C39.0696 1.52973 40.949 0 43.1855 0C45.8365 0 47.9855 2.14903 47.9855 4.8ZM45.5855 4.8C45.5855 6.12546 44.511 7.2 43.1855 7.2C41.8601 7.2 40.7855 6.12546 40.7855 4.8C40.7855 3.47452 41.8601 2.4 43.1855 2.4C44.511 2.4 45.5855 3.47452 45.5855 4.8ZM5.98554 14.2C7.31105 14.2 8.38553 13.1255 8.38553 11.8C8.38553 10.4745 7.31105 9.40002 5.98554 9.40002C4.66006 9.40002 3.58554 10.4745 3.58554 11.8C3.58554 13.1255 4.66006 14.2 5.98554 14.2ZM17.9855 14.2C19.311 14.2 20.3855 13.1255 20.3855 11.8C20.3855 10.4745 19.311 9.40002 17.9855 9.40002C16.6601 9.40002 15.5855 10.4745 15.5855 11.8C15.5855 13.1255 16.6601 14.2 17.9855 14.2ZM38.3855 21.6C39.711 21.6 40.7855 20.5255 40.7855 19.2C40.7855 17.8745 39.711 16.8 38.3855 16.8C37.0601 16.8 35.9855 17.8745 35.9855 19.2C35.9855 20.5255 37.0601 21.6 38.3855 21.6Z"
        fill="currentColor"
      />
    </svg>
  )
}

/** Notion — официальный логотип */
export function NotionBrandIcon({ size = defaultSize, style }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 33 34" fill="none" xmlns="http://www.w3.org/2000/svg" style={style}>
      <path
        d="M3.8051 3.26755L20.5301 2.04319C22.5839 1.86808 23.1124 1.98538 24.4032 2.91756L29.7421 6.64773C30.623 7.28917 30.9165 7.46381 30.9165 8.16307V28.6217C30.9165 29.9038 30.4468 30.6622 28.804 30.7782L9.38138 31.9442C8.14825 32.0027 7.56135 31.8279 6.91556 31.0114L2.98395 25.9405C2.27947 25.0072 1.98651 24.3088 1.98651 23.4918V5.3068C1.98651 4.25826 2.45649 3.38366 3.8051 3.26755Z"
        fill="#fff"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M3.64643 1.29903L20.3723 0.0746037C21.3849 -0.0114809 22.3097 -0.0595444 23.1918 0.139197C24.141 0.353054 24.86 0.807308 25.5578 1.31054L30.9002 5.04319L30.9158 5.05461C30.9547 5.08281 30.9968 5.11312 31.0417 5.14536C31.3674 5.37943 31.8354 5.71564 32.1631 6.09295C32.7252 6.73997 32.9031 7.45237 32.9031 8.16303V28.6217C32.9031 29.4467 32.763 30.5442 31.967 31.4425C31.1549 32.3592 30.0175 32.6721 28.9448 32.7479L28.9343 32.7486L9.48857 33.916L9.47602 33.9165C8.79263 33.949 8.01197 33.9383 7.24718 33.6609C6.41395 33.3586 5.82508 32.8277 5.35391 32.2318L5.34799 32.2243L1.40271 27.1359L1.39499 27.1257C0.55231 26.0092 0 24.8994 0 23.4918V5.30675C0 4.51862 0.17342 3.55089 0.82429 2.72219C1.51537 1.84231 2.52546 1.39554 3.6337 1.30013L3.64643 1.29903Z"
        fill="#fff"
      />
      <path
        d="M20.5301 2.04318C22.5838 1.86808 23.1124 1.98541 24.4031 2.91757L29.7421 6.64778C30.623 7.28918 30.9167 7.46383 30.9167 8.16301V28.6217C30.9167 29.9039 30.4468 30.6622 28.804 30.7782L9.38127 31.944C8.14822 32.0025 7.56137 31.8279 6.9156 31.0114L2.98396 25.9405C2.27951 25.0072 1.98647 24.3088 1.98645 23.492V5.30687C1.98645 4.25835 2.45646 3.38365 3.80508 3.26754L20.5301 2.04318ZM28.9214 9.91165C28.9214 9.15462 28.6285 8.74625 27.9818 8.80449L8.91064 9.91165C8.20688 9.97045 7.9722 10.3204 7.9722 11.0779V28.4466C7.97222 29.3801 8.44147 29.7293 9.49759 29.6715L27.7471 28.6217C28.8037 28.5641 28.9214 27.922 28.9214 27.1636V9.91165Z"
        fill="var(--notion-logo-fill, #000)"
      />
    </svg>
  )
}

/** ClickUp — иконка из favicon (внешний URL) */
export function ClickUpIcon({ size = defaultSize, style }: IconProps): ReactNode {
  return (
    <img
      src="https://clickup.com/assets/favicons/apple-touch-icon.png"
      alt="ClickUp"
      width={size}
      height={size}
      style={{ objectFit: 'contain', ...style }}
    />
  )
}

/** Huntflow — иконка (официальный favicon) */
export function HuntflowIcon({ size = defaultSize, style }: IconProps): ReactNode {
  return (
    <img
      src="https://huntflow.ru/favicon.ico"
      alt="Huntflow"
      width={size}
      height={size}
      style={{ objectFit: 'contain', ...style }}
    />
  )
}
