...
import {
  ...
  DAY_KEYS,
  DAY_NAMES,
} from '@/shared/lib/types/working-hours'
...
function getScheduleForDisplay(userData: ProfileInfoProps['userData']): WorkingHours['custom'] | null {
...
function shouldShowScheduleTable(schedule: WorkingHours['custom'] | null): boolean {
        ...
        <Grid columns="2" gap="4" width="100%" className={styles.grid}>
          <Box>
            ...
            <InfoRow
              ...
              copyable
            />
            <InfoRow label="Дата регистрации:" value={userData.registrationDate} inline />
            <InfoRow label="Дата последнего входа:" value={userData.lastLoginDate} inline />
          </Box>
          <Box>
            {(() => {
              ...
              <>
                ...
                <InfoRow
                  ...
                  value={formatMeetingIntervalByDay(userData.workTimeByDay, `${userData.meetingInterval} мин`)}
                  icon={<ClockIcon width={16} height={16} />}
                />
              </>
            )
