...
const REMINDER_PHRASE_ENDPOINT = 'v1/accounts/users/reminder-phrase/'

export async function fetchReminderPhrase(): Promise<string> {
  try {
    const res = await apiClient.get<ApiWrapper<{ reminder_phrase: string }>>(REMINDER_PHRASE_ENDPOINT)
    const data = res && typeof res === 'object' && 'data' in res ? (res as ApiWrapper<{ reminder_phrase: string }>).data : null
    return (data?.reminder_phrase ?? '').trim()
  } catch {
    return ''
  }
}
...
export async function saveReminderPhrase(phrase: string): Promise<boolean> {
  try {
    await apiClient.post<ApiWrapper<{ reminder_phrase: string }>>(REMINDER_PHRASE_ENDPOINT, {
      reminder_phrase: phrase.trim(),
    })
    return true
  } catch {
    return false
