  ...
  return (
    <Box className={styles.settingsContainer}>
      <Flex align="center" gap="3" mb="5">
        <Button variant="soft" size="2" onClick={() => navigate({ to: '/calendar' })}>
          <ChevronLeftIcon width={16} height={16} />
          Назад
        </Button>
        <Flex align="center" gap="2">
          <GearIcon width={20} height={20} />
          <Text size="5" weight="bold">Настройки календаря</Text>
        </Flex>
      </Flex>
        ...
        <Box p="5">
          ...
          <Flex direction="column" gap="4" className={styles.section}>
            <Box className={styles.settingRow}>
              <Box>
                <Text size="2" weight="medium" as="div">Вид по умолчанию</Text>
                <Text size="1" color="gray" as="div" mt="1">Какой вид открывать при загрузке</Text>
              </Box>
              <Select.Root value={settings.defaultView} onValueChange={(v) => updateSetting('defaultView', v as 'month' | 'week' | 'day')}>
            ...
            <Box className={styles.settingRow}>
              <Box>
                <Text size="2" weight="medium" as="div">Первый день недели</Text>
                <Text size="1" color="gray" as="div" mt="1">С какого дня начинать неделю</Text>
              </Box>
              <Select.Root value={settings.firstDayOfWeek} onValueChange={(v) => updateSetting('firstDayOfWeek', v as 'monday' | 'sunday')}>
                <Select.Trigger className={styles.selectTrigger} />
                <Select.Content>
          ...
          <Flex direction="column" gap="4" className={styles.section}>
            <Box className={styles.settingRow}>
              <Box>
                <Text size="2" weight="medium" as="div">Цветовая схема</Text>
                <Text size="1" color="gray" as="div" mt="1">По какому признаку раскрашивать события</Text>
              </Box>
              <Select.Root value={settings.eventColorScheme} onValueChange={(v) => updateSetting('eventColorScheme', v as 'byType' | 'byStatus' | 'byVacancy')}>
                <Select.Trigger className={styles.selectTrigger} />
                <Select.Content>
          ...
          <Text size="2" weight="bold" className={styles.sectionTitle}>Дополнительно</Text>
          <Flex direction="column" gap="1" className={styles.section}>
            <Flex align="center" justify="between" className={styles.switchRow}>
              <Box>
                <Text size="2" weight="medium" as="div">Показывать выходные</Text>
                <Text size="1" color="gray" as="div" mt="1">Суббота и воскресенье в календаре</Text>
              </Box>
              <Switch checked={settings.showWeekends} onCheckedChange={(v) => updateSetting('showWeekends', v)} size="2" />
            </Flex>
            <Flex align="center" justify="between" className={styles.switchRow}>
              <Box>
                <Text size="2" weight="medium" as="div">Компактный вид</Text>
                <Text size="1" color="gray" as="div" mt="1">Уменьшить размер ячеек и событий</Text>
              </Box>
              <Switch checked={settings.compactView} onCheckedChange={(v) => updateSetting('compactView', v)} size="2" />
            </Flex>
          </Flex>
        </Box>
      </Card>
