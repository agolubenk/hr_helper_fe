...
import ViewInviteModal from '@/components/invites/ViewInviteModal'
...
function InvitesPageContent() {
  ...
  const clearViewParam = () => setSearchParams((prev) => { const p = new URLSearchParams(prev); p.delete('view'); return p }, { replace: true })
  const clearEditParam = () => setSearchParams((prev) => { const p = new URLSearchParams(prev); p.delete('edit'); return p }, { replace: true })

  // Массив всех инвайтов
  const [invites, setInvites] = useState<Invite[]>(MOCK_INVITES)
  const inviteForView = viewId ? invites.find((i) => String(i.id) === viewId) ?? null : null
  const inviteForEdit = editId ? invites.find((i) => String(i.id) === editId) ?? null : null
  ...
  const handleDelete = (id: number) => {
    toast.showWarning('Удалить инвайт?', 'Вы уверены, что хотите удалить этот инвайт?', {
      duration: 0,
      actions: [
  ...
  const handleCopyInvitation = async (id: number) => {
    try {
      ...
      const mockText = invite
        ? `Инвайт для ${invite.candidate_name}, вакансия ${invite.vacancy_title}, ${invite.interview_datetime_formatted || invite.interview_datetime}. (Мок)`
        : 'Текст инвайта (мок)'
  ...
  return (
    <>
      ...
      <ViewInviteModal
        isOpen={!!viewId}
        invite={inviteForView}
        onClose={clearViewParam}
      />
      <EditInviteModal
        isOpen={!!editId}
        invite={inviteForEdit}
