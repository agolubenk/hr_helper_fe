...
import { Box, Flex, Text, Button, SegmentedControl, DropdownMenu, Dialog, Checkbox } from '@radix-ui/themes'
...
import { HamburgerMenuIcon, DownloadIcon, UploadIcon, DotsHorizontalIcon, MixerHorizontalIcon, ArrowUpIcon, ArrowDownIcon } from '@radix-ui/react-icons'
import styles from './styles/HiringRequestsPage.module.css'

const REQUEST_TABLE_COLUMNS: { id: string; label: string }[] = [
...
/** Столбцы, которые всегда отображаются (чекбокс недоступен для снятия) */
const ALWAYS_VISIBLE_COLUMN_IDS = ['vacancyGrade', 'recruiter', 'status', 'candidate', 'actions']
 ...
 */
function HiringRequestsPageContent() {
  ...
  // Модалка выбора видимых столбцов таблицы
  const [isColumnsModalOpen, setIsColumnsModalOpen] = useState(false)
  // Видимость столбцов (по умолчанию все включены)
  const [visibleColumns, setVisibleColumns] = useState<Record<string, boolean>>(() =>
    REQUEST_TABLE_COLUMNS.reduce((acc, c) => ({ ...acc, [c.id]: true }), {} as Record<string, boolean>)
  )
  // Порядок столбцов (можно менять в модалке)
  const [columnOrder, setColumnOrder] = useState<string[]>(() => REQUEST_TABLE_COLUMNS.map(c => c.id))
  ...
  const filteredRequests = mockRequests.filter(request => {
    ...
    const matchesSearch = !q ||
      ...
      (request.department && request.department.toLowerCase().includes(q)) ||
      (request.project != null && String(request.project).toLowerCase().includes(q)) ||
      (request.grade && request.grade.toLowerCase().includes(q))
  ...
  return (
    <Box className={styles.requestsContainer}>
        ...
        <Flex justify="between" align="center" className={styles.sectionHeader}>
          <Flex align="center" gap="2" className={styles.sectionHeaderLeft}>
            <HamburgerMenuIcon width={20} height={20} />
            <Text size="5" weight="bold" className={styles.sectionTitleText}>Заявки</Text>
          </Flex>
          <Flex align="center" gap="3">
            ...
            <Button
              ...
              title="Настроить отображаемые столбцы"
              className={styles.columnsButton}
            >
              <Flex align="center" gap="2">
                <MixerHorizontalIcon width={16} height={16} />
                <Text as="span" className={styles.columnsButtonText}>Столбцы</Text>
              </Flex>
            </Button>
            ...
            >
              <span>+</span>
              <span className={styles.createButtonText}> Создать заявку</span>
            </Button>
          </Flex>
        </Flex>
...
<RequestsTable
        ...
        <Dialog.Root open={isColumnsModalOpen} onOpenChange={setIsColumnsModalOpen}>
          <Dialog.Content style={{ maxWidth: 400 }}>
            <Dialog.Title>Отображаемые столбцы</Dialog.Title>
            <Dialog.Description size="2" color="gray" mb="4">
              Выберите столбцы и их порядок в таблице заявок.
            </Dialog.Description>
            <Flex direction="column" gap="2">
              {columnOrder.map((id, index) => {
                const label = REQUEST_TABLE_COLUMNS.find(c => c.id === id)?.label ?? id
                const isAlwaysVisible = ALWAYS_VISIBLE_COLUMN_IDS.includes(id)
                return (
            ...
            <Flex gap="2" justify="end" mt="4">
              <Dialog.Close>
                <Button variant="soft">Готово</Button>
              </Dialog.Close>
            </Flex>
          </Dialog.Content>
        </Dialog.Root>
...
export function HiringRequestsPage() {
  return <HiringRequestsPageContent />
}
