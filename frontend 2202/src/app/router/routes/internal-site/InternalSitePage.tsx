/**
 * Внутренний сайт — блог для сотрудников.
 * Карусель, посты, комментарии, роли пользователя и администратора.
 */
...
import { Carousel, type CarouselSlide } from '@/shared/components/internal-site/Carousel'
import { fetchBlogPosts } from '@/shared/api/internal-site'
import type { BlogPost } from '@/shared/types/internal-site'
...
export function InternalSitePage() {
  ...
  if (carouselSlides.length === 0) {
    carouselSlides.push({
      ...
      description: 'Внутренний портал для сотрудников. Новости, объявления и быстрый доступ к разделам.',
    })
  }

  const { role, setRole } = useInternalSiteRole()
    ...
    <Flex direction="column" gap="6" className={styles.container}>
      <Flex justify="between" align="start" wrap="wrap" gap="3">
        ...
        <Flex align="center" gap="3">
          ...
          {isAdmin && (
            <Link to="/internal-site/post/create">
              <Button size="2">
      ...
      <Flex gap="4" wrap="wrap">
        {QUICK_LINKS.map((item) => (
          <Link key={item.href} to={item.href} className={styles.cardLink}>
      ...
      <Card className={styles.infoCard}>
        <Flex direction="column" gap="2">
          ...
          <Text size="2" color="gray">
            Внутренний сайт — блог для сотрудников. Здесь публикуются новости и объявления, можно комментировать посты.
            Редактирование доступно авторам и администраторам.
          </Text>
        </Flex>
      </Card>
    </Flex>
