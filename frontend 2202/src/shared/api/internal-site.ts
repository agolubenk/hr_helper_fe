/**
 * API внутреннего сайта (блог).
 * Моковые данные для разработки.
 */
import type { BlogPost, BlogPostComment, BlogPostFormData } from '@/shared/types/internal-site'
...
let mockPosts: BlogPost[] = [
  {
    ...
    content: '<p>Внутренний сайт — это единая точка входа для всех сотрудников. Здесь вы найдёте новости, объявления и быстрый доступ к разделам системы.</p>',
  ...
  },
  {
    ...
    content: '<p>Раздел Вики теперь доступен всем сотрудникам. Вы можете искать статьи по категориям, тегам и приложениям.</p>',
    author: author(1, 'Голубенко Андрей'),
  ...
  },
  {
    ...
    content: '<p>В настройках календаря можно подключить Google Calendar. Слоты будут создаваться автоматически на основе рабочего времени.</p>',
...
export async function fetchBlogPosts(params?: { pinned?: boolean }): Promise<BlogPost[]> {
  await delay(200)
  let result = mockPosts.filter((p) => p.is_published)
  if (params?.pinned) result = result.filter((p) => p.is_pinned)
  return result.sort(
    (a, b) =>
...
export async function fetchBlogPost(slug: string): Promise<BlogPost | null> {
  await delay(150)
  return mockPosts.find((p) => p.slug === slug && p.is_published) ?? null
}
...
export async function fetchBlogPostComments(postId: number): Promise<BlogPostComment[]> {
  await delay(100)
  return mockComments.filter((c) => c.post_id === postId).sort(
