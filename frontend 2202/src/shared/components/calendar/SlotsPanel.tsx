    ...
    <Box className={styles.weekSection}>
      <Flex align="center" gap="2" className={styles.weekHeader} onClick={onToggle} style={{ cursor: 'pointer' }}>
        ...
        <Box style={{ marginLeft: 'auto' }}>
          <ChevronUpIcon width={16} height={16} style={{ transform: expanded ? 'rotate(0deg)' : 'rotate(180deg)', transition: 'transform 0.2s ease-in-out' }} />
        </Box>
      </Flex>
      {expanded && (
            ...
            <Box key={index} className={styles.slotCard}>
              <Flex align="center" justify="between" className={styles.slotCardHeader}>
                <Flex align="center" gap="2">
                  <Text size="3" weight="bold" style={{ textTransform: 'uppercase', color: '#ffffff' }}>{slot.dayOfWeek}</Text>
                  <Text size="3" weight="bold" style={{ color: '#ffffff' }}>{slot.date}</Text>
                </Flex>
                <Box className={styles.badge} style={{ backgroundColor: slot.count > 0 ? 'var(--gray-2)' : 'var(--gray-5)', color: slot.count > 0 ? 'var(--accent-9)' : 'var(--gray-11)' }}>
                  <Text size="1" weight="bold">{slot.count}</Text>
                </Box>
              </Flex>
              <Box style={{ padding: '12px 16px' }}>
                {slot.count > 0 ? (
                  <Text size="2" color="gray">{slot.slots.join(', ')}</Text>
                ) : (
                  <Text size="2" color="gray">Нет свободных слотов</Text>
                )}
              </Box>
            </Box>
          ))}
