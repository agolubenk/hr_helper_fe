...
import { getHuntflowUserSettings, saveHuntflowUserSettings, type HuntflowCredentialSource } from '@/shared/lib/huntflowUserSettings'
            ...
            <Box>
              ...
              <Select.Root
                ...
                onValueChange={(value) => {
                  if (value === 'disabled') {
                    toast.showWarning(
                      'Отключить интеграцию Huntflow?',
                      'Подтвердите отключение. Ключи и токены использоваться не будут.',
                      {
                        duration: 5 * 60 * 1000,
                        actions: [
                          {
  ...
  return (
    <Box className={styles.modalOverlay} onClick={onClose}>
      <Box className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        {/* Содержимое */}
        <Flex className={styles.modalContentWrapper}>
          <Box className={styles.settingsContent}>
            {renderSettings()}
          </Box>
          <Box className={`${styles.instructionsWrapper} ${isInstructionsExpanded ? styles.instructionsWrapperExpanded : ''}`}>
            ...
            <Box className={`${styles.instructions} ${!isInstructionsExpanded ? styles.instructionsCollapsed : styles.instructionsExpanded}`}>
              {renderInstructions()}
            </Box>
          </Box>
        </Flex>
