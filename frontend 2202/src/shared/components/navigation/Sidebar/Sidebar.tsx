...
function isItemOrChildrenActive(item: MenuItem, pathname: string | null | undefined): boolean {
  ...
  // 'employees' активен на страницах сотрудников, специализаций, внутренних вакансий, команд
  ...
  if (item.id === 'module-settings' && (pathname?.startsWith('/settings') || pathname?.startsWith('/interviewers') || pathname?.startsWith('/company-settings/recruiting') || pathname?.startsWith('/company-settings/candidate-fields') || pathname?.startsWith('/company-settings/scorecard') || pathname?.startsWith('/company-settings/vacancy-prompt') || pathname?.startsWith('/company-settings/sla') || pathname === '/candidate-responses')) {
    return true
  }
  ...
  const handleClick = (e?: React.MouseEvent) => {
    ...
    if (item.href) {
      ...
      if (item.external) {
        ...
        navigate({ to: item.href }) // Выполняем навигацию
...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  /** Пункты настроек после Separator (profile, integrations, module-settings-personal, company-settings, workflows, admin) */
    ...
    >
      <Flex direction="column" p="2" gap="1">
        ...
        {settingsItemsAfterSeparator.map((item) => {
          ...
          } else if (item.id === 'module-settings-personal' && pathname?.startsWith('/settings/modules')) {
            isActive = true
                ...
                <Flex align="center" className={styles.adminButtonGroup} asChild>
                  <div>
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        navigate({ to: profileRequestsHref })
                        onClose()
                      }}
                    >
