...
export default function FloatingActions({ actions = [] }: FloatingActionsProps) {
  ...
  const [quickButtons, setQuickButtons] = useState(() => getQuickButtons())
  ...
  useEffect(() => {
    ...
    const checkAndUpdateScrollTopState = () => {
      try {
        ...
        setIsScrollTopEnabled((prev) => (prev !== newValue ? newValue : prev))
      } catch {}
    }
    const checkAndUpdateSettingsState = () => {
      try {
        const saved = localStorage.getItem(SETTINGS_BUTTON_STORAGE_KEY)
        const newValue = saved !== null ? saved === 'true' : true
        setIsSettingsEnabled((prev) => (prev !== newValue ? newValue : prev))
      } catch {}
    }
  ...
  const smoothScrollTo = (element: HTMLElement | Window, target: number, duration: number = 800) => {
    return new Promise<void>((resolve) => {
      const start = element === window ? (window.scrollY || window.pageYOffset) : (element as HTMLElement).scrollTop
      ...
      const animateScroll = (currentTime: number) => {
        ...
        if (element === window) window.scrollTo(0, currentPosition)
        else (element as HTMLElement).scrollTop = currentPosition
  ...
  const scrollToTop = async () => {
    ...
    try {
      ...
      if ((document.documentElement?.scrollTop || 0) > 0) await smoothScrollTo(document.documentElement!, 0, duration)
  ...
  const buttonsToUse = actions.length > 0 ? [] : quickButtons.sort((a, b) => a.order - b.order)
  const defaultActions: FloatingAction[] = !isQuickButtonsEnabled ? [] : (actions.length > 0 ? actions : buttonsToUse.map((button) => ({
    id: button.id,
    ...
    onClick: () => {
      ...
      else if (button.type === 'text') navigator.clipboard.writeText(button.value)
      else if (button.type === 'datetime') navigator.clipboard.writeText(button.value)
    },
  })))
  ...
  const settingsAction = isSettingsEnabled ? { id: 'settings', icon: <GearIcon width="20" height="20" style={{ color: '#ffffff' }} />, onClick: handleSettingsClick, label: 'Настройки' } : null
  const scrollTopAction = isScrollTopEnabled ? { id: 'scroll-top', icon: <ArrowUpIcon width="20" height="20" style={{ color: '#ffffff' }} />, onClick: scrollToTop, label: 'Наверх' } : null
  const pinAction = { id: 'pin', icon: isPinned ? <PinLeftIcon width={20} height={20} color={pinIconColor} /> : <PinUnpinnedIcon width={20} height={20} color={pinIconColor} />, onClick: handlePinToggle, label: isPinned ? 'Открепить' : 'Закрепить' }

  const scrollableActions = defaultActions
  type ActionItem = { id: string; icon: React.ReactNode; onClick?: () => void; label?: string }
  const renderActionButton = (action: ActionItem) => (
    <Box
      ...
      style={{
        ...
        flexShrink: 0,
        borderRadius: '50%',
        backgroundColor: action.id === 'pin' && isPinned ? 'var(--gray-4)' : action.id === 'scroll-top' ? 'var(--accent-9)' : action.id === 'settings' ? 'var(--gray-9)' : buttonColorMap[action.id] || 'var(--gray-3)',
        ...
        >
          <Flex
            ...
            style={{
              ...
              maxHeight: `calc(100vh - ${topOffset === '112px' ? 202 : 184}px)`,
              overflow: 'hidden',
              display: 'flex',
          ...
          >
            ...
            >
              {scrollableActions.map((action) => renderActionButton(action))}
            </Box>
            {isScrollTopEnabled && (
              <>
                <Separator size="2" my="2" style={{ width: '100%', flexShrink: 0 }} />
                {scrollTopAction && renderActionButton(scrollTopAction)}
              </>
            )}
            <Separator size="2" my="2" style={{ width: '100%', flexShrink: 0 }} />
