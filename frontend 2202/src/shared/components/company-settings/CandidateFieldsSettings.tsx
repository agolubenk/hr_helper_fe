/**
 * CandidateFieldsSettings - управление дополнительными полями кандидатов
 */
import {
  Box,
  Flex,
...
const mockFields: CandidateField[] = [
  { id: '1', name: 'Возраст', type: 'number', required: false, order: 1, placeholder: 'Введите возраст' },
  { id: '2', name: 'Пол', type: 'select', required: false, order: 2, options: ['Мужской', 'Женский', 'Не указано'] },
  { id: '3', name: 'Зарплатные ожидания', type: 'text', required: false, order: 3, placeholder: 'Введите зарплатные ожидания' },
  { id: '4', name: 'Опыт работы', type: 'number', required: false, order: 4, placeholder: 'Введите опыт работы в годах' },
  { id: '5', name: 'LinkedIn профиль', type: 'url', required: false, order: 5, placeholder: 'https://linkedin.com/in/username' },
]
          ...
          <Table.Body>
            {fields.map((field, index) => (
              <Table.Row key={field.id}>
                ...
                <Table.Cell>
                  {field.required ? <Text color="green">Да</Text> : <Text color="gray">Нет</Text>}
                </Table.Cell>
                <Table.Cell>
                  <Flex gap="2">
                    <Button size="1" variant="soft" onClick={() => handleEditField(field)}>
                      <Pencil2Icon width={14} height={14} />
                    </Button>
                    <Button size="1" variant="soft" color="red" onClick={() => handleDeleteField(field.id)}>
      ...
      <Dialog.Root open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <Dialog.Content style={{ maxWidth: '600px' }}>
          ...
          <Flex direction="column" gap="3" mt="4">
            <Box>
              <Text size="2" weight="medium" mb="2" style={{ display: 'block' }}>Название поля *</Text>
              <TextField.Root
                value={formData.name || ''}
            ...
            <Box>
              <Text size="2" weight="medium" mb="2" style={{ display: 'block' }}>Тип поля *</Text>
              <Select.Root
                value={formData.type}
                onValueChange={(value) => {
                  setFormData({
            ...
            {formData.type === 'select' && (
              <Box>
                <Text size="2" weight="medium" mb="2" style={{ display: 'block' }}>Варианты выбора</Text>
                <Flex gap="2" mb="2">
                  <TextField.Root
                    value={newOption}
                  ...
                  <Flex direction="column" gap="1">
                    {formData.options.map((option, index) => (
                      <Flex key={index} align="center" gap="2" style={{ padding: '8px', backgroundColor: 'var(--gray-2)', borderRadius: '4px' }}>
                        ...
                        <Button size="1" variant="ghost" color="red" onClick={() => handleRemoveOption(index)}>
            ...
            {(formData.type === 'text' || formData.type === 'number' || formData.type === 'textarea' || formData.type === 'url') && (
              <Box>
                <Text size="2" weight="medium" mb="2" style={{ display: 'block' }}>Placeholder</Text>
                <TextField.Root
          ...
          <Flex gap="3" justify="end" mt="4">
            <Button variant="soft" onClick={() => setIsDialogOpen(false)}>Отмена</Button>
