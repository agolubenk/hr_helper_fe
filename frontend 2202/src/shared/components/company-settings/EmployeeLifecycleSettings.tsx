  ...
  return (
    ...
    >
      ...
      {isEditing ? (
        ...
        <>
          <Text size="2" className={styles.stageName} onClick={onStartRename}>
            {stage.name}
          </Text>
          {stage.isSystem && (
...
export default function EmployeeLifecycleSettings() {
  ...
  const handleDelete = (id: string) => {
    ...
    toast.showWarning('Удалить этап?', 'Вы уверены, что хотите удалить этот этап?', {
      actions: [
        ...
        {
          label: 'Удалить',
          onClick: () => setStages((prev) => prev.filter((x) => x.id !== id)),
          variant: 'solid',
          color: 'red',
        },
      ],
  ...
  const handleDragEnd = (blockId: string) => (event: DragEndEvent) => {
    ...
    if (draggedStage?.isSystem) return
  ...
  return (
    <Box>
      <Text size="2" color="gray" mb="4" style={{ display: 'block' }}>
        Обязательные этапы можно переименовать. Дополнительные этапы можно создавать и удалять (в блоке ЧС только
        этап «Черный список», его можно переименовать). Порядок можно менять перетаскиванием за ручку слева.
        Обязательные этапы нельзя перемещать — их относительный порядок зафиксирован.
      </Text>
          ...
          return (
            <Card key={block.id} className={styles.blockCard}>
              <Flex align="center" justify="between" mb="3">
                ...
                {block.id === 'recruiting' && (
                  <Link to="/company-settings/recruiting/stages">
              ...
              <DndContext sensors={sensors} onDragEnd={handleDragEnd(block.id)}>
                <SortableContext
