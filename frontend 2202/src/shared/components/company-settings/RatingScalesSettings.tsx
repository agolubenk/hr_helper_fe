...
import type { RatingScale, RatingScaleOption } from '@/shared/lib/ratingScale'
import { DEFAULT_RATING_SCALES } from '@/shared/lib/ratingScale'
...
export default function RatingScalesSettings() {
  ...
  const setDefault = (id: string) => {
    setScales((prev) => prev.map((s) => ({ ...s, isDefault: s.id === id })))
  }
  ...
  const updateOption = (index: number, field: 'value' | 'label', val: string) => {
    setFormOptions((prev) => prev.map((o, i) => (i === index ? { ...o, [field]: val } : o)))
  }
  ...
  const handleSave = () => {
    ...
    if (isCreating) {
      ...
      setScales((prev) =>
        prev.map((s) =>
          s.id === editingId ? { ...s, name: formName.trim(), options: options.map((o, i) => ({ ...o, order: i + 1 })) } : s
        )
      )
      setEditingId(null)
    }
      ...
      >
        <Dialog.Content style={{ maxWidth: 480 }}>
          ...
          <Flex direction="column" gap="3" mt="3">
            ...
            <Text size="2" weight="medium">
              Пункты шкалы (значение и подпись)
            </Text>
