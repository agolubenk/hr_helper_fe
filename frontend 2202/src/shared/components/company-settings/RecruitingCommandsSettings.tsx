...
function checkCommandUniqueness(
  ...
  const exactMatch = existingCommands.find(cmd => cmd.command === command && cmd.id !== excludeId)
  if (exactMatch) return { isUnique: false, duplicateCommand: exactMatch }
  ...
  if (allowAnyLayout) {
    ...
    if (otherLayoutMatch) return { isUnique: false, duplicateCommand: otherLayoutMatch }
    ...
    const loadCommands = async () => {
      try {
        ...
        const mockCommands: Command[] = [
          { id: '1', command: '/s', commandType: 'analysis', stageId: 'hr-screening', allowAnyLayout: false, color: '#22C55E', description: 'HR скрининг кандидата', order: 1 },
          { id: '2', command: '/t', commandType: 'event', stageId: 'tech-screening', allowAnyLayout: false, color: '#F97316', description: 'Технический скрининг', order: 2 },
          { id: '3', command: '/in', commandType: 'event', stageId: 'final-interview', allowAnyLayout: false, color: '#6366F1', description: 'Финальное интервью', order: 3 },
        ]
            ...
            {loading ? (
              ...
              commands.map(command => (
                <Table.Row key={command.id}>
                  <Table.Cell>
                    <Text weight="bold" style={{ fontFamily: 'monospace' }}>{command.command}</Text>
                  </Table.Cell>
                  <Table.Cell>
                  ...
                  <Table.Cell>
                    <Flex align="center" gap="2">
                      ...
                      />
                      <Text size="1" style={{ fontFamily: 'monospace' }}>{command.color}</Text>
                    </Flex>
                  </Table.Cell>
                  <Table.Cell>
                    <Text size="2" color="gray">{command.description || '-'}</Text>
                  </Table.Cell>
                  <Table.Cell>
                    <Flex gap="2">
                      <Button size="1" variant="ghost" onClick={() => handleEdit(command)}>
                        <Pencil2Icon width="14" height="14" />
                      </Button>
                      <Button size="1" variant="ghost" color="red" onClick={() => handleDelete(command.id)}>
      ...
      <Dialog.Root open={showDialog} onOpenChange={setShowDialog}>
        <Dialog.Content style={{ maxWidth: '500px' }}>
          ...
          <Flex direction="column" gap="4" mt="4">
            <Box>
              <Text size="2" weight="bold" mb="2" style={{ display: 'block' }}>Команда *</Text>
              <TextField.Root
            ...
            <Box>
              <Text size="2" weight="bold" mb="2" style={{ display: 'block' }}>Тип команды *</Text>
              <Select.Root
              ...
              >
                ...
                <Select.Content>
                  {commandTypes.map(ct => (
                    <Select.Item key={ct.value} value={ct.value}>{ct.label}</Select.Item>
                  ))}
                </Select.Content>
              </Select.Root>
              <Text size="1" color="gray" mt="1" style={{ display: 'block' }}>
            ...
            <Box>
              <Text size="2" weight="bold" mb="2" style={{ display: 'block' }}>Этап найма *</Text>
              {stages.length === 0 ? (
                <Text size="2" color="gray">Загрузка этапов...</Text>
              ) : (
                <Select.Root
                ...
                >
                  ...
                  <Select.Content>
                    {stages.map(stage => (
                      <Select.Item key={stage.id} value={stage.id}>{stage.name}</Select.Item>
                    ))}
                  </Select.Content>
                </Select.Root>
              )}
            ...
            <Box>
              <Flex align="center" gap="2" mb="2">
                ...
                />
                <Text size="2" weight="bold">Любая раскладка</Text>
              </Flex>
              <Text size="1" color="gray" mt="1" style={{ display: 'block', marginLeft: '28px' }}>
                {formData.allowAnyLayout
                  ? 'Команда будет работать независимо от раскладки клавиатуры'
                  : 'Команда будет работать только при точном совпадении'}
              </Text>
            </Box>

            <Box>
              <Text size="2" weight="bold" mb="2" style={{ display: 'block' }}>Цвет *</Text>
            ...
            <Box>
              <Text size="2" weight="bold" mb="2" style={{ display: 'block' }}>Описание</Text>
              <TextField.Root
          ...
          <Flex gap="3" mt="6" justify="end">
            <Button variant="soft" onClick={handleCancel}>Отмена</Button>
