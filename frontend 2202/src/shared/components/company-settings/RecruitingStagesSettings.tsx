...
const mockRejectionReasons: RejectionReason[] = [
  { id: '1', name: 'Не подходит по опыту', description: 'Кандидат не имеет достаточного опыта работы' },
  { id: '2', name: 'Не подходит по навыкам', description: 'Отсутствуют необходимые технические навыки' },
  { id: '3', name: 'Зарплатные ожидания слишком высокие', description: 'Запрашиваемая зарплата превышает бюджет' },
  { id: '4', name: 'Не подходит по локации', description: 'Кандидат не может работать в требуемой локации' },
  { id: '5', name: 'Другая причина', description: 'Прочие причины отказа' },
]

const mockStages: HiringStage[] = [
  { id: '1', name: 'New', order: 1, color: '#2180A0', description: 'Новый кандидат', autoTransition: false, rejectionReasonIds: [] },
  { id: '2', name: 'Under Review', order: 2, color: '#3B82F6', description: 'На рассмотрении', autoTransition: false, rejectionReasonIds: [] },
  { id: '3', name: 'Interview', order: 3, color: '#8B5CF6', description: 'Интервью', autoTransition: false, rejectionReasonIds: [] },
  { id: '4', name: 'Offer', order: 4, color: '#22C55E', description: 'Предложение', autoTransition: false, rejectionReasonIds: [] },
  { id: '5', name: 'Accepted', order: 5, color: '#10B981', description: 'Принят', autoTransition: false, rejectionReasonIds: [] },
  { id: '6', name: 'Rejected', order: 6, color: '#EF4444', description: 'Отказ', autoTransition: false, rejectionReasonIds: ['1', '2', '3', '4', '5'] },
  { id: '7', name: 'Declined', order: 7, color: '#F59E0B', description: 'Отклонено кандидатом', autoTransition: false, rejectionReasonIds: [] },
  { id: '8', name: 'Archived', order: 8, color: '#6B7280', description: 'Архив', autoTransition: false, rejectionReasonIds: [] },
]
  ...
  const handleSaveReason = () => {
    ...
    if (editingReason) {
      setRejectionReasons(rejectionReasons.map(r =>
        r.id === editingReason.id ? { ...r, ...reasonFormData } : r
      ))
    } else {
    ...
    <Box>
      <Tabs.Root value={activeTab} onValueChange={(value) => setActiveTab(value as 'stages' | 'reasons')}>
        <Flex justify="between" align="center">
          ...
          <Button variant="soft" size="2" asChild>
            <Link to="/company-settings/employee-lifecycle">
                ...
                <Table.Body>
                  {stages.map((stage, index) => (
                    <Table.Row key={stage.id}>
                      ...
                      <Table.Cell>
                        <Button size="1" variant="soft" onClick={() => handleEditStage(stage)}>
                ...
                <Table.Body>
                  {rejectionReasons.map((reason) => (
                    <Table.Row key={reason.id}>
                      ...
                      <Table.Cell>
                        <Flex gap="2">
                          <Button size="1" variant="soft" onClick={() => handleEditReason(reason)}>
                            <Pencil2Icon width={14} height={14} />
                          </Button>
                          <Button
                            size="1"
      ...
      <Dialog.Root open={isStageDialogOpen} onOpenChange={setIsStageDialogOpen}>
        <Dialog.Content style={{ maxWidth: '700px' }}>
          ...
          <Flex direction="column" gap="3" mt="4">
                  ...
                  <Tooltip.Provider>
                    <Tooltip.Root>
                      ...
                      <Tooltip.Portal>
                        ...
                        >
                          Этапы-встречи используются в тогглере на странице /workflow.
            ...
            <Box>
              <Text size="2" weight="medium" mb="2" style={{ display: 'block' }}>Описание</Text>
              <TextField.Root
            ...
            <Box>
              <Text size="2" weight="medium" mb="2" style={{ display: 'block' }}>Цвет</Text>
            ...
            {stageFormData.isMeeting && (
              <Card style={{ padding: '16px', backgroundColor: 'var(--gray-2)' }}>
                ...
                <Flex direction="column" gap="3">
                  <Box>
                    <Text size="2" weight="medium" mb="2" style={{ display: 'block' }}>Отображать офисы?</Text>
                    <Select.Root
                  ...
                  <Box>
                    <Text size="2" weight="medium" mb="2" style={{ display: 'block' }}>Отображать интервьюеров?</Text>
                    <Select.Root
          ...
          <Flex gap="3" justify="end" mt="4">
            <Button variant="soft" onClick={() => setIsStageDialogOpen(false)}>Отмена</Button>
      ...
      <Dialog.Root open={isReasonDialogOpen} onOpenChange={setIsReasonDialogOpen}>
        <Dialog.Content style={{ maxWidth: '600px' }}>
          ...
          <Flex direction="column" gap="3" mt="4">
            <Box>
              <Text size="2" weight="medium" mb="2" style={{ display: 'block' }}>Название причины отказа *</Text>
              <TextField.Root
          ...
          <Flex gap="3" justify="end" mt="4">
            <Button variant="soft" onClick={() => setIsReasonDialogOpen(false)}>Отмена</Button>
