...
\item \textbf{RecrChat (полный объём):}
  ...
  \item Моки: расширенные поля кандидата (social, rating); полная структура MOCK\_CONVERSATIONS (avatar, lastMessage, timestamp, channel, favourite). Application Details с полем Rating.
