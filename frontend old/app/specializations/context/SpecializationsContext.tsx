...
import type { SpecializationNode, GradingConfig, SpecializationHistoryEntry } from '../types'
import {
  findNodeById,
  updateNodeInTree,
  flattenTree,
...
import { INITIAL_TREE, INITIAL_GRADING } from '../data/initialTree'
...
export function SpecializationsProvider({
  ...
  const updateNode = useCallback((id: string, patch: Partial<SpecializationNode>) => {
    setTree((prev) =>
      updateNodeInTree(prev, id, (node) => ({ ...node, ...patch }))
    )
  }, [])

  const getFlatNodesForSelect = useCallback(
    (excludeId?: string) => {
      const base = [{ id: '', name: '— Без родителя —', depth: 0 }, ...flatNodes]
      if (excludeId) return base.filter((n) => n.id !== excludeId)
      return base
    },
    [flatNodes]
  )

  const getFilteredTree = useCallback((query: string) => {
    return filterTreeByQuery(cloneTree(tree), query)
  }, [tree])
  ...
  return (
    <SpecializationsContext.Provider value={value}>
      {children}
