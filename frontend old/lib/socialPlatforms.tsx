...
export interface SocialPlatformConfig {
  ...
  placeholder: string
  /** Опциональный валидатор значения (true = валидно) */
...
> = {
  whatsapp: {
    ...
    validator: (v) => /^\+?[0-9]{10,15}$/.test(v.replace(/\s/g, '')),
  },
  viber: {
    label: 'Viber',
    color: '#665CAC',
  ...
  telegram: {
    ...
    validator: (v) => /^[a-zA-Z0-9_]{5,32}$/.test(v.replace(/^@/, '')),
  },
  wechat: {
    label: 'WeChat',
    color: '#09BB07',
  ...
  linkedin: {
    ...
    validator: (v) => /^[a-zA-Z0-9-]+$/.test(v.replace(/^.*linkedin\.com\/in\//, '').replace(/\/$/, '')),
  },
  xing: {
    label: 'Xing',
    color: '#006567',
  ...
  github: {
    ...
    validator: (v) => /^[a-zA-Z0-9-]+$/.test(v.replace(/^.*github\.com\//, '').replace(/\/$/, '')),
  },
  gitlab: {
    label: 'GitLab',
    color: '#FC6D26',
