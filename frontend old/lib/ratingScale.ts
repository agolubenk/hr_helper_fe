...
/** Список шкал оценок (из настроек компании; пока мок). */
...
/** Сохранить выбранную шкалу оценок для специализации (матрица). */
...
/** Сохранить выбранную шкалу для таба специализации (тех/повед/задачи/тесты). */
...
/** Шкала по умолчанию для формы оценки (пока без API — берём из мока). */
export function getDefaultRatingScale(): RatingScale {
  return DEFAULT_RATING_SCALES.find((s) => s.isDefault) ?? DEFAULT_RATING_SCALES[0]
}

/**
 * Шкала оценок для специализации (для страницы оценки).
 * Читает выбор из матрицы (localStorage). Если не задан — шкала по умолчанию.
 */
...
/** Выбранный id шкалы для специализации (для матрицы). */
export function getRatingScaleIdForSpec(specId: string, tab?: string): string {
  if (typeof window === 'undefined') return DEFAULT_RATING_SCALES.find((s) => s.isDefault)?.id ?? DEFAULT_RATING_SCALES[0].id
  const key = tab ? STORAGE_KEY_PREFIX + specId + '_' + tab : STORAGE_KEY_PREFIX + specId
  const id = window.localStorage.getItem(key)
