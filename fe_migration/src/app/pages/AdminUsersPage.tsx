import { Box, Button, Callout, Card, Flex, Section, Table, Text, Badge } from '@radix-ui/themes'
...
export function AdminUsersPage() {
  ...
  const loadUsers = async () => {
    ...
    setError(null)
    try {
      const res = await fetch(getApiUrl('accounts/users/'), { credentials: 'include' })
        ...
        {users.length === 0 ? (
          <Flex direction="column" align="center" gap="3" py="8" px="4">
            ...
            <Text size="1" color="gray">
              Данные загружаются из API `/api/v1/accounts/users/`
            </Text>
          </Flex>
        ) : (
          <Table.Root size="2">
            ...
            <Table.Body>
              {users.map((u) => (
                <Table.Row key={u.id}>
                  ...
                  <Table.Cell>{u.full_name || [u.first_name, u.last_name].filter(Boolean).join(' ') || '—'}</Table.Cell>
                  <Table.Cell>{u.is_active ? <Badge color="green">Да</Badge> : <Badge color="gray">Нет</Badge>}</Table.Cell>
                  <Table.Cell>
                    {u.is_superuser ? (
