...
export default function FinanceSettingsPage() {
  ...
  const handleAddTax = () => {
    if (!newTax.name || !newTax.rate || !newTax.country) {
      toast.showError('Ошибка', 'Заполните все поля')
      return
    }
    
    const rate = parseFloat(newTax.rate)
    if (isNaN(rate) || rate < 0 || rate > 100) {
      toast.showError('Ошибка', 'Ставка должна быть числом от 0 до 100')
      return
    }
  ...
  const handleAddCurrency = () => {
    if (!newCurrency.code || !newCurrency.name) {
      toast.showError('Ошибка', 'Заполните код и название валюты')
      return
    }
  ...
  const handleAddAPISource = () => {
    if (!newAPISource.name || !newAPISource.url) {
      toast.showError('Ошибка', 'Заполните название и URL API источника')
      return
    }
