...
import { Flex, Text, Button, Box, TextField, Select, Badge, Table, TextArea, Callout } from '@radix-ui/themes'
import { PlusIcon, Pencil1Icon, TrashIcon, CheckIcon, Cross2Icon, MagnifyingGlassIcon, InfoCircledIcon } from '@radix-ui/react-icons'
import { useState, useEffect } from 'react'
import { useToast } from '@/components/Toast/ToastContext'
import styles from './AttractionRulesSettings.module.css'
 ...
 */
export function AttractionRulesSettings() {
  ...
  const handleAddRule = async () => {
    if (!newRule.name?.trim() || !newRule.source) {
      toast.showWarning('Обязательные поля', 'Укажите название и источник')
      return
    }
    ...
    try {
      ...
      const created: AttractionRule = {
        id: `rule-${Date.now()}`,
        name: newRule.name.trim(),
        description: (newRule.description ?? '').trim(),
        source: newRule.source,
        bonus_amount: newRule.bonus_amount ?? null,
        bonus_currency: newRule.bonus_currency ?? 'RUB',
        is_active: newRule.is_active ?? true,
        priority: newRule.priority ?? 0,
        conditions: (newRule.conditions ?? '').trim(),
        created_at: now,
        updated_at: now,
      }
      ...
      toast.showSuccess('Создано', 'Правило добавлено')
    } catch {
  ...
  const handleEditRule = async (rule: AttractionRule) => {
    ...
    try {
      ...
      const now = new Date().toISOString()
      setRules((prev) =>
        prev.map((r) => (r.id === rule.id ? { ...rule, updated_at: now } : r)),
      )
      setEditingRule(null)
      toast.showSuccess('Сохранено', 'Правило обновлено')
    } catch {
  ...
  const handleDeleteRule = (id: string) => {
    toast.showWarning('Удалить правило?', 'Вы уверены, что хотите удалить это правило?', {
      actions: [
        ...
        { label: 'Удалить', onClick: () => void performDeleteRule(id), variant: 'solid', color: 'red' },
      ],
    })
  }
  ...
  return (
    <Flex direction="column" gap="4" className={styles.page}>
        ...
        <Callout.Root color="amber" mb="2">
          ...
          <Callout.Text size="2">
            Раздел запланирован к полной переработке. В новом плане нужно явно разделить: аспекты и поля привлечения,
            механизмы, процессы, связь с правилами фильтрации и отсева из других модулей, прочие параметры. Подробнее:
            репозиторий — файл{' '}
