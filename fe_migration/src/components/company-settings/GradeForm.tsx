...
interface GradeFormProps {
  ...
  /** Порядок полей (кроме названия) — как в таблице и в «Порядок столбцов». */
  columnOrder: string[]
...
export default function GradeForm({
  ...
  const handleAdditionalFieldChange = (fieldId: string) => (e: React.ChangeEvent<HTMLInputElement>) => {
    setFormData((prev) => ({
      ...prev,
      additionalFields: {
  ...
  const renderMiddleField = (colKey: string): ReactNode => {
    switch (colKey) {
      case 'category':
        return (
          <Box>
    ...
    <form onSubmit={handleSubmit}>
      <Flex direction="column" gap="4">
        ...
        {columnOrder.map((colKey) => {
          const fieldBlock = renderMiddleField(colKey)
          if (!fieldBlock) {
            return null
          }
          return <Fragment key={colKey}>{fieldBlock}</Fragment>
        })}
