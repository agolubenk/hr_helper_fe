...
import { Box, Flex, Text, Button, Dialog, TextField, TextArea, Select, Card, Switch, Popover } from "@radix-ui/themes"
import { PlusIcon, InfoCircledIcon, CalendarIcon, FileTextIcon, PersonIcon, Cross2Icon, ChevronDownIcon } from "@radix-ui/react-icons"
...
export default function CreateRequestModal({ isOpen, onClose, onSave }: CreateRequestModalProps) {
  ...
  const [additionalRecruiters, setAdditionalRecruiters] = useState<Set<string>>(new Set())
  const [additionalRecruitersPopoverOpen, setAdditionalRecruitersPopoverOpen] = useState(false)
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      setAdditionalRecruiters(new Set())
      setAdditionalRecruitersPopoverOpen(false)
    }
  }, [isOpen])
              ...
              <Flex direction="column" gap="4">
                ...
                <Box>
                  <Flex align="center" justify="between" mb="2">
                    ...
                    <Flex align="center" gap="2">
                      <Text size="1" color="gray">Ввести вручную</Text>
                      <Switch checked={deadlineManual} onCheckedChange={setDeadlineManual} />
                    </Flex>
                  </Flex>
                  {deadlineManual ? (
                    <TextField.Root
                ...
                <Box>
                  ...
                  <Text size="2" weight="medium" mt="4" mb="2" style={{ display: 'block' }}>
                    Дополнительные рекрутеры
                  </Text>
                  <Popover.Root open={additionalRecruitersPopoverOpen} onOpenChange={setAdditionalRecruitersPopoverOpen}>
                    <Popover.Trigger>
                      <Button variant="soft" color="gray" style={{ width: '100%', justifyContent: 'space-between' }}>
                        <Text size="2" color="gray">Выберите рекрутеров...</Text>
                        <ChevronDownIcon width={16} height={16} />
                      </Button>
                    </Popover.Trigger>
                    <Popover.Content style={{ maxHeight: 280, overflowY: 'auto' }}>
                      <Flex direction="column" gap="1">
                        {mockRecruiters
                          .filter(r => r !== formData.recruiter)
                          .map(recruiter => (
                            <Button
                              key={recruiter}
                              variant="ghost"
                              ...
                              onClick={() => {
                                setAdditionalRecruiters(prev => new Set(prev).add(recruiter))
                                setAdditionalRecruitersPopoverOpen(false)
                              }}
                            >
                              {recruiter}
                        ...
                        {mockRecruiters.filter(r => r !== formData.recruiter).length === 0 && (
                          <Text size="1" color="gray">Выберите основного рекрутера</Text>
                        )}
                      </Flex>
                    </Popover.Content>
                  </Popover.Root>
                  <Flex wrap="wrap" gap="2" mt="2">
                    {Array.from(additionalRecruiters)
                      .filter(r => r !== formData.recruiter)
                      .map(recruiter => (
                        <Box
                          key={recruiter}
                        ...
                        >
                          <Text size="1">{recruiter}</Text>
                          <button
                            type="button"
                            aria-label={`Удалить ${recruiter}`}
                            onClick={() => setAdditionalRecruiters(prev => { const n = new Set(prev); n.delete(recruiter); return n })}
                            style={{
                              background: 'none',
                              border: 'none',
                              padding: 0,
