...
const MEETING_INTERVALS = Array.from({ length: 13 }, (_, i) => i * 5)
...
export function ScheduleSettingsPage({ initialData, onCancel, onSave }: ScheduleSettingsPageProps) {
  ...
  const defaultInterval =
    initialData.meetingInterval && MEETING_INTERVALS.includes(Number(initialData.meetingInterval))
      ? initialData.meetingInterval
      : '15'
  const normalize = (w?: WorkTimeByDay): WorkTimeByDay => {
    if (!w || Object.keys(w).length === 0) {
      return DAY_KEYS.reduce<WorkTimeByDay>((acc, day) => {
        acc[day] = {
          ...
          isWorkday: normalizeDay(day),
          meetingInterval: defaultInterval,
        }
        return acc
      }, {})
    }
    return DAY_KEYS.reduce<WorkTimeByDay>((acc, day) => {
      const ex = w[day] as
        | { start?: string; end?: string; isWorkday?: boolean; meetingInterval?: string }
        | undefined
      acc[day] = {
        start: ex?.start ?? defaultStart,
        end: ex?.end ?? defaultEnd,
        isWorkday: ex?.isWorkday ?? normalizeDay(day),
        meetingInterval: ex?.meetingInterval ?? defaultInterval,
      }
      return acc
    }, {})
  }
  ...
  const handleDayChange = (
    ...
    field: 'start' | 'end' | 'isWorkday' | 'meetingInterval',
                  ...
                  return (
                    <Box key={day} className={styles.dayRow}>
                      <Flex align="center" gap="3" mb="2">
                        <Switch
                          checked={s.isWorkday}
                          onCheckedChange={(v) => handleDayChange(day, 'isWorkday', v)}
                        />
                        <Text size="2" weight="medium">
                          {DAY_NAMES[day]}
                        </Text>
                      ...
                      <Grid columns="3" gap="4" width="100%" className={styles.dayGrid}>
                        ...
                        <Box>
                          ...
                          <Select.Root
                            value={
                              s.meetingInterval &&
                              MEETING_INTERVALS.includes(Number(s.meetingInterval))
                                ? s.meetingInterval
                                : '15'
                            }
            ...
            {isAllDaysSame && (
              <Box style={{ marginTop: '16px' }}>
                <Text
                  size="2"
                  weight="medium"
                  style={{ marginBottom: '8px', display: 'block', color: 'var(--gray-11)' }}
                >
