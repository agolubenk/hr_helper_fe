...
**Профиль в fe_migration (доп. к эталону 3001):** часть экрана доведена под «целевую платформу», но исходный `frontend/` из репозитория удалён. Полный перечень: **`docs/UI_MIGRATION_PLAN.md`**. Отличия от чистого копирования 3001: **UserCard** — без строки Telegram в карточке (ник в соцсетях ниже по странице); **заглушка аватара** — рамка + тень (как у загруженного фото).
...
| **`fe_migration/docs/UI_MIGRATION_PLAN.md`** | Сводка UI по профилю/футеру/FloatingActions и т.д. (исторически сравнивалось с `frontend`, который удалён). |
...
| **Миграция обновлённого меню** | В `fe_migration` поддерживать актуальность меню по требованиям продукта (источник `frontend` удалён). |
